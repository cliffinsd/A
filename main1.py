...
def accessory_menu():
    ...
    cursor.execute("SELECT DISTINCT name FROM rentals WHERE type = 'accessory'")
...
def guard_menu():
    keyboard = InlineKeyboardMarkup(row_width=1)
    cursor.execute("SELECT DISTINCT name FROM rentals WHERE type = 'guard'")
...
def calculate_earnings(user_id, period):
    cursor.execute("SELECT type, amount, hours FROM rentals WHERE user_id = ?", (user_id,))
...
def callback_query(call):
    ...
    if call.data == "add_accessory":
        bot.edit_message_text("📦 Введите название аксессуара:", call.message.chat.id, call.message.message_id)
        user_states[user_id] = "waiting_acc_name"
    elif call.data == "add_guard":
        bot.edit_message_text("🛡️ Введите имя охранника:", call.message.chat.id, call.message.message_id)
        user_states[user_id] = "waiting_guard_name"
    elif call.data == "list_accessories":
        bot.edit_message_text("📋 <b>Список аксессуаров</b>", call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=accessory_menu())
    elif call.data == "list_guards":
        bot.edit_message_text("👥 <b>Список охранников</b>", call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=guard_menu())
    elif call.data == "earnings":
        bot.edit_message_text(format_earnings_message(user_id), call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=main_menu())
    elif call.data.startswith("acc_"):
        acc_name = call.data[4:]
        bot.edit_message_text(f"📦 <b>{acc_name}</b>\nВыберите сумму аренды за час:", call /

.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=amount_menu("acc", acc_name))
    elif call.data.startswith("guard_"):
        guard_name = call.data[6:]
        bot.edit_message_text(f"🛡️ <b>{guard_name}</b>\nВыберите сумму аренды за час:", call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=amount_menu("guard", guard_name))
        ...
        bot.edit_message_text(f"{('📦' if item_type == 'acc' else '🛡️')} <b>{name}</b>\nСумма: {amount}\nВыберите количество часов:", 
                             call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=hours_menu(item_type, name, amount))
        ...
        db.commit()
        bot.edit_message_text(f"✅ Добавлена аренда: <b>{name}</b> на {hours} часов за {amount}", 
                             call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=main_menu())
    elif call.data == "back":
        bot.edit_message_text("👋 <b>Главное меню</b>", call.message.chat.id, call.message.message_id, parse_mode="HTML", reply_markup=main_menu())
    bot.answer_callback_query(call.id)
...
def handle_text(message):
    ...
    if state == "waiting_acc_name":
        ...
        if cursor.fetchone():
            bot.send_message(message.chat.id, "⚠️ Аксессуар с таким названием уже существует", reply_markup=main_menu())
        else:
            cursor.execute("INSERT INTO rentals (user_id, type, name, amount, hours, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
        ...
        if cursor.fetchone():
            bot.send_message(message.chat.id, "⚠️ Охранник с таким именем уже существует", reply_markup=main_menu())
        else:
            cursor.execute("INSERT INTO rentals (user_id, type, name, amount, hours, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
